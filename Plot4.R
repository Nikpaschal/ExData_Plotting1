# Plot4.R comprises of a series of functions and commands to download and subset Electrical 
...
# A vector of the form c(nr, nc). Subsequent figures will be drawn in an nr-by-nc array 
# on the device by  rows (mfrow)
par(mfrow=c(2,2))

# plot functionS
#TOP LEFT CORNER (conrtucted with plot2.R)
with(Feb2007_power, (plot(Feb2007_power$Time,as.numeric(as.character(Feb2007_power$Global_active_power)),type="l",  xlab="",ylab="Global Active Power")))  

#TOP RIGHT CORNER
with(Feb2007_power, (plot(Feb2007_power$Time,as.numeric(as.character(Feb2007_power$Voltage)), type="l",xlab="datetime",ylab="Voltage")))

#BOTTOM LEFT CORNER (conrtucted with plot3.R), with a cex= argument to adjust legend size
...
#BOTTOM RIGHT CORNER
plot(Feb2007_power$Time,as.numeric(as.character(Feb2007_power$Global_reactive_power)),type="l",xlab="datetime",ylab="Global_reactive_power")
